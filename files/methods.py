...
def user_allowed_to_upload(request):
    ...
    # Fix: user_allowed_to_comment guards CAN_COMMENT with hasattr() but this
    # function accessed CAN_ADD_MEDIA directly, raising AttributeError on any
    # deployment that omits the setting from a custom config. Use getattr() with
    # a safe default of "all" to match the documented default behaviour.
    can_add = getattr(settings, "CAN_ADD_MEDIA", "all")
    if can_add == "all":
        return True
    elif can_add == "email_verified":
        if request.user.email_is_verified:
            return True
    elif can_add == "advancedUser":
