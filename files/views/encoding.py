...
class EncodingDetail(APIView):
    ...
    def post(self, request, encoding_id):
        ...
        if action == "start":
            ...
            try:
                ...
                return Response({"status": "fail"}, status=status.HTTP_400_BAD_REQUEST)
            # Fix: updating fields on an existing resource should return 200 OK,
            # not 201 Created. 201 is reserved for newly created resources.
    ...
    def put(self, request, encoding_id, format=None):
        ...
        encoding.save()
        # Fix: replacing the file on an existing Encoding record is an update,
        # not a creation — 200 OK is the semantically correct response code.
