    ...
    def get_object(self, friendly_token):
        try:
            ...
            if media.state == "private" and self.request.user != media.user:
                # Fix: was returning 400 Bad Request for private-media access denial.
                # 403 Forbidden is the correct status: the user is authenticated but
                # lacks permission; 400 implies a malformed request, which is misleading.
                return Response({"detail": "media is private"}, status=status.HTTP_403_FORBIDDEN)
            return media
        except PermissionDenied:
            # Fix: PermissionDenied should yield 403, not 400.
