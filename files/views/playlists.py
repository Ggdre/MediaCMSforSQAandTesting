...
class PlaylistDetail(APIView):
    ...
    def get_playlist(self, friendly_token):
        try:
            ...
            # Fix: PermissionDenied should yield 403 Forbidden, not 400 Bad Request.
    ...
    def get(self, request, friendly_token, format=None):
        ...
        # Fix #1370: previously only the playlist owner could see private media
        # inside a playlist. Users who hold an explicit MediaPermission or belong
        # to an RBAC group that grants access to a private item were incorrectly
        # blocked. Now we include private media for authenticated users who have
        # member-level access, matching the access model used elsewhere in the API.
        if request.user.is_authenticated and request.user == playlist.user:
            ...
            allowed_private_ids = [
                pm.media_id
                for pm in PlaylistMedia.objects.filter(playlist=playlist, media__state="private").select_related("media__user")
                if request.user.has_member_access_to_media(pm.media)
            ]
            playlist_media = (
                PlaylistMedia.objects.filter(playlist=playlist)
                .filter(Q(media__state="public") | Q(media__state="unlisted") | Q(media__id__in=allowed_private_ids))
            ...
            if media:
                if action == "add":
                    ...
                    if media_in_playlist >= settings.MAX_MEDIA_PER_PLAYLIST:
                        ...
                        # Fix: passing ordering= as a lookup kwarg caused duplicates
                        # when the media was already in the playlist at a different
                        # position. Use defaults= so ordering is set only on INSERT.
                    ...
                    # Fix: removing an item is not a creation — 200 OK is correct.
                    return Response(
                    ...
                    if ordering:
                        ...
                        # Fix: updating ordering is not a creation — 200 OK is correct.
                        return Response(
