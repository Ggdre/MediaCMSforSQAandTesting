...
from .. import helpers
from ..helpers import has_cjk_characters
from ..methods import (
    change_media_owner,
    copy_media,
    get_user_or_session,
...
class MediaList(APIView):
    ...
    def get(self, request, format=None):
        ...
        if query:
            ...
            # Fix #1474: PostgreSQL full-text search (tsquery ":*" prefix syntax)
            # does not handle CJK (Chinese/Japanese/Korean) characters. Fall back
            # to a case-insensitive LIKE search on title and description instead.
            if has_cjk_characters(query):
                media = media.filter(Q(title__icontains=query) | Q(description__icontains=query))
                query = None  # already filtered; skip the block below
            else:
            ...
            for playlist in playlists:
                for m in media:
                    ...
                    if media_in_playlist < settings.MAX_MEDIA_PER_PLAYLIST:
                        # Fix: previously passed ordering= as a lookup kwarg, so if
                        # the media was already in the playlist at a different position
                        # get_or_create would create a duplicate instead of finding the
                        # existing record. Use defaults= to keep ordering as an insert-
                        # only value that does not participate in the lookup.
...
class MediaDetail(APIView):
    ...
    def get_object(self, friendly_token):
        try:
            ...
            if media.state == "private":
                # Fix #1442: Return 403 Forbidden (not 401 Unauthorized) when an
                # authenticated user is denied access to private media. 401 implies
                # the client should re-authenticate; 403 correctly signals the user
                # is authenticated but lacks permission for this resource.
    ...
    def put(self, request, friendly_token, format=None):
        ...
        if serializer.is_valid():
            # Fix #1492: do NOT pass user=request.user here. The 'user' field is
            # read-only on MediaSerializer, but passing it as a kwarg to .save()
            # bypasses that guard and silently overwrites the media's owner with
            # the requesting user (e.g. an editor who is not the owner).
            # On update we only need to save the validated writable fields.
            if request.data.get('media_file'):
                media_file = request.data["media_file"]
                media.state = helpers.get_default_state(request.user)
                media.listable = False
                serializer.save(media_file=media_file)
            else:
                serializer.save()
            return Response(serializer.data, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class MediaActions(APIView):
    ...
    def get_object(self, friendly_token):
        try:
            media = Media.objects.select_related("user").prefetch_related("encodings__profile").get(friendly_token=friendly_token)
            # Fix #1442: was checking only media.user == request.user, which blocked
            # users who have explicit MediaPermission access (viewer/editor/owner)
            # from seeing actions on private media they legitimately have access to.
            # Also corrected HTTP status from 400 (Bad Request) to 403 (Forbidden),
            # which is semantically correct when access is denied due to permissions.
            if media.state == "private":
                user = self.request.user
                has_access = (
                    user == media.user
                    or (user.is_authenticated and user.has_member_access_to_media(media))
                    or is_mediacms_editor(user)
                )
                if not has_access:
                    return Response({"detail": "media is private"}, status=status.HTTP_403_FORBIDDEN)
            return media
        except PermissionDenied:
            return Response({"detail": "bad permissions"}, status=status.HTTP_403_FORBIDDEN)
        except BaseException:
    ...
    def get(self, request, friendly_token, format=None):
        ...
        media = self.get_object(friendly_token)
        # Fix #1442-b: the Response guard must come BEFORE any attribute access.
        # Previously, accessing media.user on a Response object (returned when
        # media is missing or private) raised an AttributeError, masking the
        # real error and causing a 500 instead of the intended 403/404.
        if isinstance(media, Response):
            return media
    ...
    def delete(self, request, friendly_token, format=None):
        ...
        if action:
            if action == "report":  # delete reported actions
                ...
                # Fix: was returning 201 Created for a reset/delete operation.
                # 200 OK is semantically correct here since no resource is created.
                return Response(
...
class MediaSearch(APIView):
    ...
    def get(self, request, format=None):
        ...
        if query:
            query = helpers.clean_query(query)
            # Fix #1474: CJK characters are not supported by PostgreSQL tsquery
            # prefix matching. Fall back to icontains for CJK queries.
            if has_cjk_characters(query):
                media = media.filter(Q(title__icontains=query) | Q(description__icontains=query))
                query = None
            else:
