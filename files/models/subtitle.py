...
class Subtitle(models.Model):
    ...
    def subtitle_text(self):
        # Fix #1433: pysubs2.load() raises FileNotFoundError if the subtitle file
        # has been deleted from disk while the DB record still exists (e.g. after
        # a manual cleanup). The bare exception previously propagated up through
        # update_search_vector and silently corrupted the search index update.
        try:
...
def subtitle_delete(sender, instance, **kwargs):
    """Fix #1433-b: deleting a subtitle previously left its text permanently
    indexed in the parent media's search vector. Now we rebuild the vector
    after deletion so removed subtitle content is no longer searchable."""
    from .. import tasks
