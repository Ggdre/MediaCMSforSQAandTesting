...
def get_file_name(filename):
    """Return the base filename from a path, cross-platform (Windows uses \\)."""
...
def media_file_info(input_file):
    ...
    # File size: use Python on Windows (no stat); otherwise use stat command
    if os.name == "nt":
        try:
    ...
    # MD5: use Python on Windows (no md5sum); otherwise use md5sum command
    if os.name == "nt":
        try:
    ...
    ret = {
        ...
        # Fix #1397-a: was incorrectly reading "color_space" for both keys. The
        # wrong value caused FFmpeg to receive an incorrect color-transfer hint
        # when re-encoding, which could result in washed-out or mis-toned output.
...
def has_cjk_characters(text):
    """Return True if *text* contains any CJK (Chinese/Japanese/Korean) code points.

    PostgreSQL full-text search with the default text-search configuration does
    not tokenise CJK characters correctly, so callers should fall back to a
    plain ILIKE / icontains search when this function returns True.

    Unicode ranges covered:
    - CJK Unified Ideographs: U+4E00–U+9FFF
    - CJK Extension A: U+3400–U+4DBF
    - CJK Extension B–F: U+20000–U+2A6DF (and related)
