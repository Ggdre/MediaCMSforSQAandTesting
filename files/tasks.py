...
def update_search_vector(friendly_token):
    try:
        ...
        # Fix: a bare except clause catches BaseException including KeyboardInterrupt
        # and SystemExit, which can prevent the worker from shutting down cleanly.
        # Narrowing to Exception avoids that problem while still catching all
        # application-level errors (Media.DoesNotExist, DB errors, etc.).
        return False
