...
# SAML provider (allauth) requires python3-saml/onelogin; skip if not installed
try:
    from onelogin.saml2.auth import OneLogin_Saml2_Settings  # noqa: F401
...
# Disable connection pool when testing so Django test runner doesn't wait 30s for a connection
_db_opts = {"pool": True} if not os.environ.get("TESTING") else {}
DATABASES = {"default": {"ENGINE": "django.db.backends.postgresql", "NAME": "mediacms", "HOST": "127.0.0.1", "PORT": os.environ.get("POSTGRES_PORT", "5432"), "USER": "mediacms", "PASSWORD": "mediacms", "OPTIONS": _db_opts}}
...
# When connecting from host to Docker: POSTGRES_PORT set => use localhost and disable pool
# (local_settings may set HOST='db' and Redis to 'redis', which only resolve inside Docker)
